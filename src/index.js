...
io.on('connection', socket => {
    ...
    socket.on("sendMessage", (message, callback) => {
        ...
        if(player) {
            ...
            callback(); // invoke the callback to trigger event acknowledgement
        }
    });
    ...
    socket.on("sendAnswer", (answer, callback) => {
        ...
        if(player) {
            ...
            // Since we want to show the player's submission to the rest of the players, we have to emit
            // an event (`answer`) to all the players in the room along with the player's answer and `isRoundOver`.
